...
# Este script vive en la raíz de ControlesFinancieros/ y se autoubica.
# Path(__file__).parent apunta siempre al directorio donde vive el script
# independientemente de desde dónde se ejecute — sin Path.cwd() frágil.
# Cambiar REPO_NOMBRE solo tiene efecto informativo (mensajes y prefijos).
REPO_NOMBRE = "ControlesFinancieros"  # informativo — para mensajes y prefijos

# Directorio raíz = donde vive este script — hardcodeado por diseño
...
def crear_estructura(repo_nombre: str, proyecto: str) -> tuple:
    """
    Crea la estructura de directorios y ficheros stub.
    Usa REPO_DIR_OVERRIDE = Path(__file__).parent — el script se autoubica
    en la raíz del repo independientemente de desde dónde se ejecute.
...
if __name__ == "__main__":
    try:
        ...
        print(f"Raiz:     {REPO_DIR_OVERRIDE}")
        print(f"Proyecto: {PROYECTO}")
        print("=" * 50)

        n_dirs, n_ficheros, omitidos = crear_estructura(
            REPO_NOMBRE, PROYECTO
        )
